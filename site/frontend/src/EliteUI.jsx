...
export default function EliteUI() {
  ...
  async function refreshDevices() {
    try {
      ...
      // Filter out backend server (hostname contains 'srv-' or 'hibernate' or starts with 'srv-d3mbstd6ubrc73elqhfg')
      const list = allDevices.filter(d => {
        const hostname = (d.hostname || '').toLowerCase()
        return !hostname.includes('srv-') && 
               !hostname.includes('hibernate') && 
               !hostname.startsWith('srv-d3mbstd6ubrc73elqhfg')
      })
      setDevices(list)
      ...
      <header className="elite-header">
        <div className="elite-brand">
          <div className="elite-logo">
            <span className="elite-logo-emoji">🐱</span>
          </div>
          <div>
